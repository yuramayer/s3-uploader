...
def test_get_file_path(monkeypatch):
    """Should return expanded path from user input."""
...
def test_normalize_path_absolute(tmp_path):
    """Should resolve absolute paths without modification."""
...
def test_normalize_path_relative(tmp_path, monkeypatch):
    """Should resolve files relative to the current directory."""
    file = tmp_path / "data.txt"
    file.write_text("content")
...
def test_normalize_path_missing(tmp_path, monkeypatch):
    """Should raise an error when the target file is absent."""
    monkeypatch.chdir(tmp_path)
